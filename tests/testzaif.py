...
class TestZaif(TestCase):
    ...
    def test_name(cls):
        ''' name test method '''
        ok_(zf.NAME == string.replace(cls.__name__, 'Test', ''))

    @classmethod
    def test_price(cls):
        ''' last price test method '''
        ok_(zf.get_current_price(zf.CCY_DEFAULT) > 0.00)

    @classmethod
    def test_bid(cls):
        ''' bid price test method '''
        ok_(zf.get_current_bid(zf.CCY_DEFAULT) > 0.00)

    @classmethod
    def test_ask(cls):
        ''' ask price test method '''
        ok_(zf.get_current_ask(zf.CCY_DEFAULT) > 0.00)
