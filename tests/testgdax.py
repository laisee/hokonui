...
class Testgdax(TestCase):
    ...
    def test_name(cls):
        ''' test name '''
        ok_(gdx.NAME == string.replace(cls.__name__, 'Test', ''))

    @classmethod
    def test_price(cls):
        ''' method  for testing last price '''
        ok_(gdx.get_current_price(base.CCY_DEFAULT) > 0.00)

    @classmethod
    def test_bid(cls):
        ''' method for testing bid price  '''
        ok_(gdx.get_current_bid(base.CCY_DEFAULT) > 0.00)

    @classmethod
    def test_ask(cls):
        ''' method for testing ask price  '''
        ok_(gdx.get_current_ask(base.CCY_DEFAULT) > 0.00)
