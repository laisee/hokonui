...
class TestCoinbase(TestCase):
    ...
    def test_name(cls):
        """{0}.{1}"""
        assert cba.NAME == cls.__name__.replace("Test", "")
    ...
    def test_ask(cls):
        """{0}.{1}"""
        assert float(cba.get_current_ask(base.CCY_DEFAULT, None)) > 0.00
    ...
    def test_ticker(cls):
        ...
        assert data["pair"] == base.CCY_DEFAULT, "pair should be base.CCY_DEFAULT"
    ...
    def test_orders(cls):
        ...
        assert orders["source"] == "Coinbase", "Source should be 'Coinbase'"
